...
public class SensorValuesChartGraph extends AbstractChart {
   ...
   */
  	...
  	public GraphicalView execute(final Activity act, final ServiceEcare service, ArrayList<ArrayList <Measure>> l, int period, final int sensor) {
		...
		view = ChartFactory.getTimeChartView(act,
				getDataset(act, l, period, sensor), 
				getRenderer(act, l, period, sensor), 
				dateFormat);
		...
		view.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				if (seriesSelection == null) {
					...
					Log.w(Constants.TAG, "CM trouvée pour ce point : "+cm.size()+" mesures");
					if(cm.size()>0){
  	...
  	private XYMultipleSeriesRenderer getRenderer(Context context, ArrayList<ArrayList <Measure>> l, int period, int sensor) {
		...
		// TODO : décaler de quelques pixels la légende verticale pr laisser une plus grande marge.
		...
		renderer.setMarginsColor(Color.rgb(232, 232, 232));
		...
		renderer.setMargins(new int[] {0, 70, 20, 10}); //(dessus,gauche,bas,droite)
